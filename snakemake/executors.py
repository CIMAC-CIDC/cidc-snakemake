...
from snakemake.utils import Unformattable, format, makedirs
...
class RealExecutor(AbstractExecutor):
    ...
    def format_job_pattern(self, pattern, job=None, **kwargs):
        ...
        if self.workflow.overwrite_configfile:
            overwrite_config.extend(
                ("--configfile", self.workflow.overwrite_configfile)
            )
...
class DRMAAExecutor(ClusterExecutor):
    ...
    def run(self, job, callback=None, submit_callback=None, error_callback=None):
        ...
        except (drmaa.InternalException, drmaa.InvalidAttributeValueException) as e:
            print_exception(
    ...
    def cancel(self):
        ...
        with self.lock:
            for j in self.active_jobs:
                self.kubeapi.delete_namespaced_pod(j.jobid, self.namespace, body)
        self.shutdown()
